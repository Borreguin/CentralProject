"""
Before running this script, please install gitpython:

pip3 install gitpython

Assume this file does not change:
git update-index --assume-unchanged subtree.py

This script updates a subtree:
...
from sb_util import verify_git_lib_install_if_needed

is_installed = verify_git_lib_install_if_needed()
if is_installed:
...
class GitExecutor:
    ...
    def __init__(self, **kwargs):
        for key, value in kwargs.items():
            if key == 'path':
                self.path = value
                self.subtreePath = os.path.join(project_path, value)
                self.subtreeName = to_snake_case(os.path.basename(value))
            else:
                setattr(self, key, value)
        self.repository = Repo.init(project_path)
...
def main():
    inputs = parse_args()
    log_this(f'Start subtree routine --> Project: {project_id}, Path: {project_path}')
